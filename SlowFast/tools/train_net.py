...
def train_epoch(
    ...
    if cfg.MIXUP.ENABLE: # 可以选择打开, 也可以选择不打开; 
        mixup_fn = MixUp(
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(
        ...
        if cfg.H2O.NOUN_VERB:
            (labels, noun_labels, verb_labels) = labels
        ...
        if cfg.NUM_GPUS:
            ...
            if not isinstance(labels, list):
                ...
                if cfg.H2O.NOUN_VERB:
                    noun_labels = noun_labels.cuda(non_blocking=True)
                    verb_labels = verb_labels.cuda(non_blocking=True)
                index = index.cuda(non_blocking=True)
                time = time.cuda(non_blocking=True)
            
            
            for key, val in meta.items(): # 遍历value相关的信息; 
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            # Explicitly declare reduction to mean. # 混合精度加速训练; 
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel":
            ...
            elif cfg.DETECTION.ENABLE: # default false value; 
                ...
                if cfg.H2O.NOUN_VERB:
                    preds, noun_preds, verb_preds = model(inputs)
                else:
                    preds = model(inputs)
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel" and partial_loss: # 自监督预训练? 
                loss = partial_loss
            else:
                if cfg.H2O.NOUN_VERB:
                    loss = loss_fun(preds, labels)
                    noun_loss = loss_fun(noun_preds, noun_labels)
                    verb_loss = loss_fun(verb_preds, verb_labels)

                    loss = loss * cfg.H2O.NOUN_VERB_WEIGHT[0] + cfg.H2O.NOUN_VERB_WEIGHT[1] * noun_loss + cfg.H2O.NOUN_VERB_WEIGHT[2] * verb_loss
                else:
                    loss = loss_fun(preds, labels)
        ...
        # check Nan Loss.
        misc.check_nan_losses(loss) # 检查是不是有异常的损失函数数值? 
        if perform_backward:
        ...
        if cfg.DETECTION.ENABLE: # False
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                if cfg.H2O.NOUN_VERB:
                    ...
                    noun_num_topks_correct = metrics.topks_correct(noun_preds, noun_labels, (1, 5))
                    ...
                    verb_num_topks_correct = metrics.topks_correct(verb_preds, verb_labels, (1, 5))
                ...
                if cfg.NUM_GPUS > 1:
                    if cfg.H2O.NOUN_VERB:
                        loss, grad_norm, top1_err, top5_err, noun_top1_err, noun_top5_err, verb_top1_err, verb_top5_err = du.all_reduce(
                            [loss.detach(), grad_norm, top1_err, top5_err, noun_top1_err, noun_top5_err, verb_top1_err, verb_top5_err]
                        )
                    else:
                ...
                if cfg.H2O.NOUN_VERB:
                    loss, grad_norm, top1_err, top5_err, noun_top1_err, verb_top1_err = (
                        loss.item(),
                        grad_norm.item(),
                        top1_err.item(),
                        top5_err.item(),
                ...
                writer.add_scalars(
                    {
                        ...
                        "Train/NounTop1_err": noun_top1_err,
                        "Train/VerbTop1_err": verb_top1_err,
                    },
                    global_step=data_size * cur_epoch + cur_iter, # 从全局的角度来看这是第几个时间步骤? 
                )
    ...
    train_meter.reset() # 仅仅统计一个Epoch的信息? 
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(val_loader):
        ...
        if cfg.NUM_GPUS:
            ...
            if isinstance(inputs, (list,)):
                ...
                inputs = inputs.cuda(non_blocking=True)
            if cfg.H2O.NOUN_VERB:
                (labels, noun_labels, verb_labels) =  (labels.cuda(), noun_labels.cuda(), verb_labels.cuda())
            else:
                labels =  labels.cuda()
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL: # 不进行多标签操作; 
                if cfg.NUM_GPUS > 1:
                    preds, labels = du.all_gather([preds, labels])
            else: 
                if cfg.DATA.IN22k_VAL_IN1K != "":
                    preds = preds[:, :1000] # 仅评估1000个样本
                ...
                if cfg.NUM_GPUS > 1:
                    ...
                    if cfg.H2O.NOUN_VERB:
                        noun_top1_err, noun_top5_err, verb_top1_err, verb_top5_err = du.all_reduce([noun_top1_err, noun_top5_err, verb_top1_err, verb_top5_err])
                ...
                if cfg.H2O.NOUN_VERB:
                    noun_top1_err, noun_top5_err, verb_top1_err, verb_top5_err = noun_top1_err.item(), noun_top5_err.item(), verb_top1_err.item(), verb_top5_err.item()
...
def train(cfg):
    ...
    # Init multigrid.
    multigrid = None
    if cfg.MULTIGRID.LONG_CYCLE or cfg.MULTIGRID.SHORT_CYCLE: # 都是False, 先不用管了; 
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        if cur_epoch > 0 and cfg.DATA.LOADER_CHUNK_SIZE > 0: # CHUNK SIZE代表批量化的数据读取; 
        ...
        if hasattr(train_loader.dataset, "_set_epoch_num"):
            train_loader.dataset._set_epoch_num(cur_epoch) # 当前是第几个EPOCHS? 
        # Train for one epoch.
