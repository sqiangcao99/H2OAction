...
def retry_load_flows(image_paths, retry=10, backend="pytorch"):
    """
    This function is to load images with support of retrying for failed load.

    Args:
        image_paths (list): paths of images needed to be loaded(图片的绝对路径).
    ...
    for i in range(retry):
        ...
        for image_path in image_paths:
            flow_x_path, flow_y_path = image_path

            with pathmgr.open(flow_x_path, "rb") as f:
                img_str = np.frombuffer(f.read(), np.uint8)
                flow_x = cv2.imdecode(img_str, flags=cv2.IMREAD_GRAYSCALE).astype(np.float32)

            with pathmgr.open(flow_y_path, "rb") as f:
                img_str = np.frombuffer(f.read(), np.uint8)
                flow_y = cv2.imdecode(img_str, flags=cv2.IMREAD_GRAYSCALE).astype(np.float32)

            flow = np.sqrt(flow_x ** 2 + flow_y ** 2)
            # img = np.stack((flow_x, flow_y, flow), axis=2).astype(np.uint8)
            img = np.stack((flow_x, flow_y), axis=2).astype(np.uint8)
...
def get_sequence(center_idx, half_len, sample_rate, num_frames):
    """
    Sample frames among the corresponding clip. # 仅对一个CLIP进行采样;

    Args:
        center_idx (int): center frame idx for current clip # 对CLIP进行定位;
        half_len (int): half of the clip length # CLIP的长度;
        sample_rate (int): sampling rate for sampling frames inside of the clip # 每个CLIP的采样率;
        num_frames (int): number of expected sampled frames # 被采样的视频帧数

    Returns:
        seq (list): list of indexes of sampled frames in this clip. # 这个CLIP中要多少帧
    """
    seq = list(range(center_idx - half_len, center_idx + half_len, sample_rate))

    for seq_idx in range(len(seq)):
        if seq[seq_idx] < 0:
            seq[seq_idx] = 0  # 采样的话使用第一帧进行补齐;
        elif seq[seq_idx] >= num_frames:
            seq[seq_idx] = num_frames - 1  # 超出的话使用最后一帧进行补齐;
    return seq  # index;
...
def spatial_sampling(
    ...
    """
    assert spatial_idx in [-2, -1, 0, 1, 2] # 验证模式取-2; 
    if spatial_idx == -1: # 代表训练模式; 
        ...
        frames, _ = transform.random_short_side_scale_jitter(
            frames, min_size=crop_size, max_size=crop_size,
        )
        frames, _ = transform.uniform_crop(frames, crop_size, spatial_idx=1) # 取中间位置的视频帧; 
    else:
