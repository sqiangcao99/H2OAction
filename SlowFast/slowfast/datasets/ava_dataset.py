...
class Ava(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, split):
        ...
        self._sample_rate = cfg.DATA.SAMPLING_RATE  # 采样率
        self._video_length = cfg.DATA.NUM_FRAMES  # 训练的长度
        self._seq_len = self._video_length * self._sample_rate  # 采集多长的片段;
    ...
    def print_summary(self):
        ...
        logger.info(
            "Number of key frames: {}".format(len(self))
        )  # 有box的帧; 定义了len函数;
    ...
    def num_videos(self):
        ...
        Returns:
            (int): the number of videos in the dataset.
        """
        return len(self._keyframe_indices)  # 关键帧的索引; 每个索引可能来自若干帧;
    ...
    def __getitem__(self, idx):
        ...
        video_idx, sec_idx, sec, center_idx = self._keyframe_indices[
            idx
        ]  ## 对元组进行解析;
        # Get the frame idxs for current clip.
        seq = utils.get_sequence(
            center_idx,  # clip位置
            self._seq_len // 2,  # clip的长度
            self._sample_rate,  # 采样率;
            num_frames=len(self._image_paths[video_idx]),  # 存了多少帧?
        )
        ...
        ]  # load images according to the img path
        ...
        return imgs, liabel_arrs, dx, torch.zeros(1), extra_data
