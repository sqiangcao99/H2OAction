...
class SlowFast(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.enable_noun_verb = cfg.H2O.NOUN_VERB
    ...
    def _construct_network(self, cfg):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if self.enable_noun_verb:
                self.head_noun = head_helper.ResNetBasicHead(
                    dim_in=[
                        width_per_group * 32,
                        width_per_group * 32 // cfg.SLOWFAST.BETA_INV,
                    ],
                    num_classes=cfg.H2O.NOUN_NUM,
                    pool_size=[None, None]
                    if cfg.MULTIGRID.SHORT_CYCLE
                    or cfg.MODEL.MODEL_NAME == "ContrastiveModel"
                    else [
                ...
                self.head_verb = head_helper.ResNetBasicHead(
                    dim_in=[
                        width_per_group * 32,
                        width_per_group * 32 // cfg.SLOWFAST.BETA_INV,
                    ],
                    num_classes=cfg.H2O.VERB_NUM,
                    pool_size=[None, None]
                    if cfg.MULTIGRID.SHORT_CYCLE
                    or cfg.MODEL.MODEL_NAME == "ContrastiveModel"
                    else [
    ...
    def forward(self, x, bboxes=None):
        ...
        if self.enable_detection:
            ...
            if self.enable_noun_verb:
                x_ = self.head(x)
                x_noun = self.head_noun(x)
                x_verb = self.head_verb(x)

                return x_, x_noun, x_verb
            else:
                x_ = self.head(x)
...
class X3D(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        if self.enable_detection:
            ...
            if self.enable_noun_verb:
                ...
                self.head_noun = head_helper.X3DHead(
                    dim_in=dim_out,
                    dim_inner=dim_inner,
                    dim_out=cfg.X3D.DIM_C5,
                    num_classes=cfg.H2O.NOUN_NUM,
                ...
                self.head_verb = head_helper.X3DHead(
                    dim_in=dim_out,
                    dim_inner=dim_inner,
                    dim_out=cfg.X3D.DIM_C5,
                    num_classes=cfg.H2O.VERB_NUM,
    ...
    def forward(self, x, bboxes=None):
        ...
        if self.enable_noun_verb:
            for module in list(self.children())[:-3]:
                x = module(x)
